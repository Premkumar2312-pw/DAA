...
class BellmanFord {
    ...
    static void bellmanFord(List<Edge> edges, int V, int src) {
        int[] dist = new int[V + 1]; // Using 1-based index
        ...
        for (int i = 1; i < V; i++) {
            ...
            for (int j = 1; j <= V; j++) { // Start from 1
                System.out.println("Vertex " + j + " Distance: " + (dist[j] == Integer.MAX_VALUE ? "INF" : dist[j]));
            }
            if (!updated) break; // Stop if no updates occur
        }
